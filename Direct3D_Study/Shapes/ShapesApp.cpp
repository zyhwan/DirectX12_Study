...
struct RenderItem
{
	...
	// 물체의 자료가 변해서 상수 버퍼를 갱신해야 하는지의 여부를 뜻하는 '더러움' 플래그.
    // FrameResource마다 물체의 cbuffer가 있으므로, FrameResource마다 갱신을 적용해야 한다.
    // 따라서, 물체의 자료를 수정할 때에는 반드시
    // NumFramesDirty = gNumFrameResources로 설정해야 한다. 그래야 각각의 프레임 자원이 갱신된다.
...
void ShapesApp::Update(const GameTimer& gt)
{
    ...
    //mCurrFrameResource의 자원들(상수 버퍼 등)을 갱신한다.
}
...
void ShapesApp::Draw(const GameTimer& gt)
{
    ...
    // 이 프레임의 명령 목록들을 구축하고 제출한다.

    // 현재 울타리 지점까지의 명령들을 표시하도록 울타리 값(Fence 멤버)을 증가시킨다.
    mCurrFrameResource->Fence = ++mCurrentFence;
    
    // 새울타리 지점을 설정하는 명령(Signal)을 명령 대기열에 추가한다.
    // 지금 우리는 GPU 시간선(timeline) 상에 있으므로, 새 울타리 지점은
    // GPU가 이 Signal() 명령 이전까지의 모든 명령을 처리하기 전까지는
    // 설정되지 않는다.
    mCommandQueue->Signal(mFence.Get(), mCurrentFence);

    // GPU가 아직도 이전 프레임들의 명령을 처리하고 있을 수 있지만,
    // 그 프레임들에 연관된 프레임 자원들은 여기서 전혀 건드리지
    // 않으므로 문제가 되지 않는다.
}
...
void ShapesApp::BuildConstantBufferViews()
{
    ...
    // 마지막 세 서술자는 각 프레임 자원의 패스별 CBV이다.
    for(int frameIndex = 0; frameIndex < gNumFrameResources; ++frameIndex)
    {
...
void ShapesApp::BuildShapeGeometry()
{
	...
	//
	// 이 예제는 모든 기하구조를 하나의 커다란 정점/인덱스 버퍼에 담는다.
    // 따라서 버퍼에서 각 부분 메시가 차지하는 영역들을 정의할 필요가 있다.
	//

	// 연결된 정점 버퍼에서의 각 물체의 정점 오프셋을
    // 적절한 변수들에 보관해 둔다.
    ...
    // 저점/인덱스 버퍼에서 각 물체가 차지하는 영역을 나타내는
...
//응용 프로그램 클래스는 프레임 자우너 세 개를 담는 벡터를 생성
//현재 프레임 자원을 가리키는 멤버 변수들도 적절히 관리한다.
...
void ShapesApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
    ...
    for(size_t i = 0; i < ritems.size(); ++i)
    {
        ...
        // 현재 프레임 자원에 대한 서술자 힙에서 이 물체를 위한
        // CBV의 오프셋을 구한다.
